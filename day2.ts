const input = `Game 1: 2 blue, 3 red; 3 green, 3 blue, 6 red; 4 blue, 6 red; 2 green, 2 blue, 9 red; 2 red, 4 blue
...
export function solution1(input: string[]) {
  ...
  let gameWinIdCount = 0
  const gameIdLost = new Set<number>()

  for (const line of input) {
    const [game, allRolls] = line.trim().split(':')
    const gameIndex = Number(game.trim().split(' ')[1])
    const allSets = allRolls.trim().split(';')
    ...
    for (const set of allSets) {
      ...
      const colourCount: Record<'blue' | 'green' | 'red', number> = {
        blue: 0,
        green: 0,
        red: 0,
      }

      for (const draw of hand) {
        const [digit, colour] = draw.trim().split(' ')
        colourCount[colour as keyof typeof colourCount] += Number(digit)
      }

      Object.entries(colourCount).forEach(([colour, count]) => {
        if (count > limits[colour as keyof typeof limits]) {
          gameIdLost.add(gameIndex)
        }
      })
    }
  }
  ...
  return gameWinIdCount
}

export function solution2(input: string[]) {
  let count = 0
  for (const line of input) {
    const [game, gameLine] = line.trim().split(':')
    const allSetsInAGame = gameLine.trim().split(';')

    const minCubesRequired: Record<'red' | 'green' | 'blue', number> = {
      red: 0,
      green: 0,
      blue: 0,
    }

    for (const set of allSetsInAGame) {
      const hand = set
        .trim()
        .split(',')
        .map((x) => x.trim())
      ...
      if (minCubesRequired.red < colourCount.red) {
        minCubesRequired.red = colourCount.red
      }

      if (minCubesRequired.green < colourCount.green) {
        minCubesRequired.green = colourCount.green
      }

      if (minCubesRequired.blue < colourCount.blue) {
        minCubesRequired.blue = colourCount.blue
      }
    }

    count +=
      minCubesRequired.red * minCubesRequired.green * minCubesRequired.blue
  }

  return count
}

console.log('Solution 1 result:', solution1(inputArr))
console.log('Solution 2 result:', solution2(inputArr))
