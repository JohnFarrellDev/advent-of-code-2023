const input = `seeds: 3640772818 104094365 1236480411 161072229 376099792 370219099 1590268366 273715765 3224333694 68979978 2070154278 189826014 3855332650 230434913 3033760782 82305885 837883389 177854788 2442602612 571881366
...
function updateStage(
  input: string,
  currentStage: Stage,
): { updatedStage: Stage; updated: boolean } {
  if (input === 'seed-to-soil map:') {
    return { updatedStage: 'seedsToSoil', updated: true }
  }
  if (input === 'soil-to-fertilizer map:') {
    return { updatedStage: 'soilToFertilizer', updated: true }
  }
  if (input === 'fertilizer-to-water map:') {
    return { updatedStage: 'fertilizerToWater', updated: true }
  }
  if (input === 'water-to-light map:') {
    return { updatedStage: 'waterToLight', updated: true }
  }
  if (input === 'light-to-temperature map:') {
    return { updatedStage: 'lightToTemperature', updated: true }
  }
  if (input === 'temperature-to-humidity map:') {
    return { updatedStage: 'temperatureToHumidity', updated: true }
  }
  if (input === 'humidity-to-location map:') {
    return { updatedStage: 'humidityToLocation', updated: true }
  }

  return { updatedStage: currentStage, updated: false }
}

function parseInput(input: string): ParsedData {
  const inputPerLine = input.split('\n').filter(Boolean)

  let stage: Stage = 'seeds'
  ...
  for (const line of inputPerLine) {
    const { updatedStage, updated } = updateStage(line, stage)
    if (updated) {
      stage = updatedStage
      continue
    }
    if (stage === 'seeds') {
      parsedData.seeds = line.split(':')[1].trim().split(' ').map(Number)
    } else {
      const [destRangeStart, sourceRangeStart, range] = line.split(' ').map(Number)
      parsedData[stage].push({
        sourceRangeStart: sourceRangeStart,
        sourceRangeEnd: sourceRangeStart + range - 1,
        destinationRangeStart: destRangeStart,
        destinationRangeEnd: destRangeStart + range - 1,
        range: range,
      })
    }
  }
...
export function solution1(input: string) {
  ...
  const seedValues = parsedInput.seeds

  const soilValues = seedValues.map((seed) => {
    const soil = parsedInput.seedsToSoil.find(
      (x) => seed >= x.sourceRangeStart && seed <= x.sourceRangeEnd,
    )
    if (!soil) return seed
    return seed - soil.sourceRangeStart + soil.destinationRangeStart
  })

  const fetilizerValues = soilValues.map((soil) => {
    const fertilizer = parsedInput.soilToFertilizer.find(
      (x) => soil >= x.sourceRangeStart && soil <= x.sourceRangeEnd,
    )
    if (!fertilizer) return soil
    return soil - fertilizer.sourceRangeStart + fertilizer.destinationRangeStart
  })

  const waterValues = fetilizerValues.map((fertilizer) => {
    const water = parsedInput.fertilizerToWater.find(
      (x) => fertilizer >= x.sourceRangeStart && fertilizer <= x.sourceRangeEnd,
    )
    if (!water) return fertilizer
    return fertilizer - water.sourceRangeStart + water.destinationRangeStart
  })

  const lightValues = waterValues.map((water) => {
    const light = parsedInput.waterToLight.find(
      (x) => water >= x.sourceRangeStart && water <= x.sourceRangeEnd,
    )
    if (!light) return water
    return water - light.sourceRangeStart + light.destinationRangeStart
  })

  const temperatureValues = lightValues.map((light) => {
    const temperature = parsedInput.lightToTemperature.find(
      (x) => light >= x.sourceRangeStart && light <= x.sourceRangeEnd,
    )
    if (!temperature) return light
    return light - temperature.sourceRangeStart + temperature.destinationRangeStart
  })

  const humidityValues = temperatureValues.map((temperature) => {
    const humidity = parsedInput.temperatureToHumidity.find(
      (x) => temperature >= x.sourceRangeStart && temperature <= x.sourceRangeEnd,
    )
    if (!humidity) return temperature
    return temperature - humidity.sourceRangeStart + humidity.destinationRangeStart
  })

  const locationValues = humidityValues.map((humdity) => {
    const location = parsedInput.humidityToLocation.find(
      (x) => humdity >= x.sourceRangeStart && humdity <= x.sourceRangeEnd,
    )
    if (!location) return humdity
    return humdity - location.sourceRangeStart + location.destinationRangeStart
  })
...
export function solution2(input: string) {
  ...
  for (let i = 0; i < parsedInput.seeds.length; i += 2) {
    const [seed, range] = [parsedInput.seeds[i], parsedInput.seeds[i + 1]]

    for (let seedValue = seed; seedValue < seed + range; seedValue++) {
      const soilMap = parsedInput.seedsToSoil.find(
        (x) => seed >= x.sourceRangeStart && seed <= x.sourceRangeEnd,
      )
      const soilValue = soilMap
        ? seedValue - soilMap.sourceRangeStart + soilMap.destinationRangeStart
        : seedValue

      const fertiLizerMap = parsedInput.soilToFertilizer.find(
        (x) => soilValue >= x.sourceRangeStart && soilValue <= x.sourceRangeEnd,
      )

      const fertilizerValue = fertiLizerMap
        ? soilValue - fertiLizerMap.sourceRangeStart + fertiLizerMap.destinationRangeStart
        : soilValue

      const waterMap = parsedInput.fertilizerToWater.find(
        (x) => fertilizerValue >= x.sourceRangeStart && fertilizerValue <= x.sourceRangeEnd,
      )

      const waterValue = waterMap
        ? fertilizerValue - waterMap.sourceRangeStart + waterMap.destinationRangeStart
        : fertilizerValue

      const lightMap = parsedInput.waterToLight.find(
        (x) => waterValue >= x.sourceRangeStart && waterValue <= x.sourceRangeEnd,
      )

      const lightValue = lightMap
        ? waterValue - lightMap.sourceRangeStart + lightMap.destinationRangeStart
        : waterValue

      const temperatureMap = parsedInput.lightToTemperature.find(
        (x) => lightValue >= x.sourceRangeStart && lightValue <= x.sourceRangeEnd,
      )

      const temperatureValue = temperatureMap
        ? lightValue - temperatureMap.sourceRangeStart + temperatureMap.destinationRangeStart
        : lightValue

      const humidityMap = parsedInput.temperatureToHumidity.find(
        (x) => temperatureValue >= x.sourceRangeStart && temperatureValue <= x.sourceRangeEnd,
      )

      const humidityValue = humidityMap
        ? temperatureValue - humidityMap.sourceRangeStart + humidityMap.destinationRangeStart
        : temperatureValue

      const locationMap = parsedInput.humidityToLocation.find(
        (x) => humidityValue >= x.sourceRangeStart && humidityValue <= x.sourceRangeEnd,
      )

      const locationValue = locationMap
        ? humidityValue - locationMap.sourceRangeStart + locationMap.destinationRangeStart
        : humidityValue

      if (locationValue < minLocation) minLocation = locationValue
    }
  }
