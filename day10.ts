const input = `FL7L-F7FJF-7FFF7FF|-F-F|JF--|.|-J-7.F--FJF|-7.F.-FF7.-F7-L-|FF-F-|-7.-F7-FF-|7J-J.F-J7.FF7..L7F|7F-7JF-L7F.|--FL7F---|.FFF-L7FL--L777L-.FF7F
...
const pipes = new Set<Pipe>(['|', '-', 'L', 'J', '7', 'F', 'S'])

const north = { y: -1, x: 0 } as const
const south = { y: 1, x: 0 } as const
const west = { y: 0, x: -1 } as const
const east = { y: 0, x: 1 } as const

const mapValueToDirection = {
  '|': [north, south],
  '-': [west, east],
  L: [north, east],
  J: [north, west],
...
function parseData(input: string) {
  const inputPerLine = input.split('\n').map((line) => line.split('')) as (Pipe | NotPipe)[][]

  const sIndex = { x: -1, y: -1 }

  for (let yIndex = 0; yIndex < inputPerLine.length; yIndex++) {
    for (let xIndex = 0; xIndex < inputPerLine[yIndex].length; xIndex++) {
      if (inputPerLine[yIndex][xIndex] === 'S') {
        sIndex.x = xIndex
        sIndex.y = yIndex
      }
    }
...
// valid if connects to two neighbours

type xCoordinate = number
type yCoordinate = number
type Visited = `x:${xCoordinate},y:${yCoordinate}`

type StackItem = { coordinate: { x: number; y: number }; stepsTakenSoFar: number }

function step(
  mapData: (Pipe | '.')[][],
  stack: StackItem[],
  coordinateToSteps: Map<Visited, number>,
) {
  while (stack.length) {
    const item = stack.pop() as StackItem

    const currentValue = mapData[item.coordinate.y]?.[item.coordinate.x]
    if (!currentValue || currentValue === '.') continue
    const visitedString = coordinateToVisited({ x: item.coordinate.x, y: item.coordinate.y })
    const previousStepsToVisit = coordinateToSteps.get(visitedString)

    if (previousStepsToVisit !== undefined && item.stepsTakenSoFar > previousStepsToVisit) {
      continue
    }

    coordinateToSteps.set(visitedString, item.stepsTakenSoFar)

    const neighbours = mapValueToDirection[currentValue]

    if (currentValue !== 'S') {
      for (const { x, y } of neighbours) {
        const newCoordinates = {
          x: item.coordinate.x + x,
          y: item.coordinate.y + y,
        }
        stack.push({
          coordinate: newCoordinates,
          stepsTakenSoFar: item.stepsTakenSoFar + 1,
        })
      }
    } else {
      const north = { y: item.coordinate.y - 1, x: item.coordinate.x }
      const east = { y: item.coordinate.y, x: item.coordinate.x + 1 }
      const south = { y: item.coordinate.y + 1, x: item.coordinate.x }
      const west = { y: item.coordinate.y, x: item.coordinate.x - 1 }

      const northValue = mapData[north.y][north.x]
      const eastValue = mapData[east.y][east.x]
      const southValue = mapData[south.y][south.x]
      const westValue = mapData[west.y][west.x]
      ...
      if (southValue === '|' || southValue === 'L' || southValue === 'J') {
        stack.push({
          coordinate: south,
          stepsTakenSoFar: item.stepsTakenSoFar + 1,
        })
...
function coordinateToVisited(coordinate: { x: number; y: number }): Visited {
...
export function solution1(input: string) {
  ...
  const stepsToVisit = new Map<Visited, number>()

  step(
    parsedData.data,
    [
      {
        coordinate: { x: parsedData.startingIndex.x, y: parsedData.startingIndex.y },
        stepsTakenSoFar: 0,
      },
    ],
    stepsToVisit,
  ...
  return Math.max(...Array.from(stepsToVisit.values()))
}

console.log('🚀 ~ file: day10.ts:14 ~ solution1 ~ solution1:', solution1(input))
